...
our ($abuseip, $accept, $apache401timeout, $apache403timeout,
	 ...
	 $systemstatstimeout, $tar, $toomanymatches, $tz, $uidtimeout,
...
if ($config{UI}) {
	# The Integrated UI - lfd's own in-process HTTPS server, and the three
	# ConfigServer display modules behind it - was removed; see CHANGES.md
	# for the whole retirement. UI and the other UI_* settings are still
	# read here for one reason only: so that a server whose csf.conf still
	# says UI = "1" is TOLD the port is gone, in the log the operator
	# already reads, rather than finding out by connecting to a port that
